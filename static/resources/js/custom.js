...
function changeSlide() {
  ...
  for (let i = 0; i < slides_length; i++) {
    ...
    if (i === current_slide) {
      jQuery("a.popup-image")
        .eq(i)
        .css({ visibility: "visible", "z-index": "2" })
        .stop()
        .animate(
          {
            opacity: 1,
...
jQuery(function($) {
  ...
  $(body).on("click", '#album-release button[data-plyr="play"]', function() {
    $('#album-release .plyr--playing button[data-plyr="pause"]').trigger(
      "click"
    )
  })
  ...
  $(".play-mp3").on("click", function() {
    ...
    if (
      ...
      $("#mp3_1").html("")
      $("#mp3_1").html(
        '<audio controls class="audio1" onended="playerItemEnd()"><source src="' +
          $(this)
            .closest(".play-pause")
            .data("src") +
  ...
  $("body").on("click", "#mp3_1 .plyr__controls button", function() {
    if ($(this).data("plyr") == "play") {
      $("#playlist")
        .find(".music_line")
        .eq(current_track - 1)
        .find(".play-pause .play-mp3")
        .hide()
      $("#playlist")
        .find(".music_line")
        .eq(current_track - 1)
        .find(".play-pause .pause-mp3")
        .show()
  ...
  $(document).on("ready", function() {
    ...
    $(".play-pause").each(function() {
      var that = $(this)
      getDuration($(this).data("src")).then(function(length) {
  ...
  $(".anim_border").on("click", function(event) {
    ...
    for (var i = 0; i < popup_images_arr.length; i++) {
      if (event.currentTarget == $(".anim_border")[i]) {
        current_slide = i
        appendBody +=
          '<a href="javascript:void(0)" class="popup-image" style="position: absolute; opacity: 1; visibility: visible"><img src=' +
          popup_images_arr[i] +
          "></a>"
      } else {
        appendBody +=
          '<a href="javascript:void(0)" class="popup-image" style="position: absolute;  opacity: 0; visibility: hidden"><img src=' +
          popup_images_arr[i] +
          "></a>"
      }
    }
  ...
  $("#popup-container").on("click", function(e) {
    if (
      !$(e.target).is($("#slides-container a img")) &&
      !$(e.target).is($("#popup-container ul li")) &&
      !$(e.target).is($("#popup_block .popup_arrow")) &&
      !$(e.target).is($("#popup_block .popup_arrow i"))
    )
  ...
  for (
    i = 0;
    i < $("#carouselExampleIndicators .carousel-inner .carousel-item").length;
    i++
  ) {
